...
def preprocess(text):
    ...
    for t,v in nltk.pos_tag(gensim.utils.simple_preprocess(text)):
        ...
        if t not in gensim.parsing.preprocessing.STOPWORDS:
            if v.startswith('N') or v.startswith('I'):
                result.append(WordNetLemmatizer().lemmatize(t.lower(), pos='n'))
            if v.startswith('V'):
                result.append(WordNetLemmatizer().lemmatize(t.lower(), pos='v'))
            if v.startswith('J'):
                result.append(WordNetLemmatizer().lemmatize(t.lower(), pos='a'))
            if v.startswith('R'):
                result.append(WordNetLemmatizer().lemmatize(t.lower(), pos='r'))
    return result
...
def sort_tfidf(bow_corpus, total_word_count, DictDocFreq):
    ...
    for a in total_word_count:
        ...
        dictTFIDF[a] = int(total_word_count[a]) * log2(D / int(DictDocFreq[a]))
...
def filter_lest_frequent(dictionary, processed_docs, no_below_percent, keep_n):
    '''
    :param dictionary: a gensim.corpora.dictionary.Dictionary. Unique words in whole corpus.
    :param processed_docs: a pandas.core.series.Series, each row is a list of words of the doc after text mining.
...
def Topic_CombinedWord_ProbTopic(topic_combined_top_words, topic_prob, source, year, num_topics):
    ...
    # save results
    result.to_csv('../result/topic/' + source + '_' + year + '_' + str(num_topics) + '_topics' + '.csv', index=False)
    return result
