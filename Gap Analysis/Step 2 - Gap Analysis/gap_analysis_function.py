...
def dict_to_matrix(dict, columnname,
    ...
    if saveheatmap == True:
        ...
        ax1.set_ylabel('')
        ax1.set_xlabel('')
        sns.heatmap(matrix).figure.savefig("../result/gap_analysis/" + save_graph_name + ".png")
        plt.close(fig1)
    return matrix


def index_as_column(matrix, save_dir = "../result/gap_analysis/", save_matrix_name = 'matrix'):
...
def to_longtable(matrix, save_dir = "../result/gap_analysis/", save_longtable_name = 'long'):
...
def plot_hist(long, save_dir = "../result/gap_analysis/", save_hist_name = 'hist_softcosine'):
